...
public class WaitingBillAdapter extends RecyclerView.Adapter<WaitingBillAdapter.ViewHolder> {

    ApiInterface service;
    private ArrayList<Bill> list;
    WaitingBillFragment waitingBillFragment;
    private Context context;
    LinearLayout bill_item;

    public WaitingBillAdapter(ArrayList<Bill> list, WaitingBillFragment waitingBillFragment) {
        this.list = list;
        this.waitingBillFragment = waitingBillFragment;
    }

    @NonNull
    @Override
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(waitingBillFragment.getContext()).inflate(R.layout.bill_item, parent, false);
        context = parent.getContext();
        service = Common.getGsonService();
        return new WaitingBillAdapter.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull WaitingBillAdapter.ViewHolder holder, int position) {
                ...
                public void onClick(View v) {
                    service.deliveringBill("" + model.getId(), "delivering").enqueue(new Callback<JSONResponseBill>() {
                        ...
                        public void onResponse(Call<JSONResponseBill> call, Response<JSONResponseBill> response) {
                            Toast.makeText(waitingBillFragment.getContext(), "Delivering!", Toast.LENGTH_SHORT).show();
                            waitingBillFragment.getFragmentManager().beginTransaction().detach(waitingBillFragment).attach(waitingBillFragment).commit();
                        }
        ...
        bill_item.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Toast.makeText(waitingBillFragment.getContext(), "Selected: " + model.getId(), Toast.LENGTH_SHORT).show();
            }
        });
